...
Feature: Burger menu offers Import / Export / Boards / Settings entries
  ...
  SPEC §4: the burger menu in the drawer holds the top-level kiosk
  actions — Import, Export, Boards (board collection management),
  Settings (per-board theme; §17.31) — plus future admin entries. It
  opens on tap of the trigger and closes on outside tap
  (`composedPath`-based, walks shadow DOM correctly so the drawer does
  NOT close at the same time when the user taps another spot in the
  drawer). The Import / Export / Boards consumers themselves wire in
  Phase 10 per SPEC §15.4 + §17.3; the Settings consumer is wired in
  §17.31. This feature only validates the menu structure + open/close
  UX.
