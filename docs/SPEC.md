...
3. **Run `git status` and `git log --oneline`** — confirm `HEAD` matches the latest commit recorded in §17.0. Phases 0–8 are fully landed (incl. the §17.13 + §17.14 + §17.15 + §17.16 + §17.17 + §17.18 + §17.19 refinements; DT-10 script half too — Tasks A/B remain manual ops); the working tree should be clean (or hold only docs/test-fixture WIP).
4. **Sanity check the build**: `npm test` (expect **467** tests across **43** files per §17.19), `npm run lint`, `npm run lint:rules`, `npm run build` (expect ~88 KB / ~25 KB gzip), and `npm run test:e2e` (expect **54** scenarios) — all should be green before starting new work. The first `npm run test:e2e` after a clone needs `npx playwright install chromium`.
