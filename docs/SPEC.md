...
| **10 (Phase 10B — Boards panel)** | — | — | TODO (next) | `<boards-panel-modal>` for collection-level list / switch / create — backed by `BoardCollectionService.{switchTo,createBoard}`. The Settings… modal stays as-is for single-board name / colour / delete (separate surfaces, per the §17.33 design decision). New `persistence/board_collection.feature`. |
| **10 (Phase 10C — routing scenarios)** | — | — | TODO | `routing/{deep_link,focus_to_url,unknown_uuid_fallback}.feature` + `persistence/load_save.feature`. The routing wiring already exists (drill / breadcrumb / unknown-uuid fallback are all live in §17.20–§17.32); this commit closes the §15.6 TP-D test plan. |
