...
- An "edit value" affordance that round-trips the markdown source through a textarea. Today the operator can author markdown only at add-child time (and via `BoardCollectionRepository` mutations); a future Phase 10 might wire an in-tile editor.
...
3. **Run `git status` and `git log --oneline`** — confirm `HEAD` matches the latest commit recorded in §17.0. Phases 0–9 are fully landed (incl. the §17.13 + §17.14 + §17.15 + §17.16 + §17.17 + §17.18 + §17.19 Phase 8 refinements, the §17.20 Phase 9 drill animation, the §17.21 Phase 9 polish — burger overflow / board-level age gradient / showcase seed, the §17.22 demo-pass tweak — `#743089` showcase fresh colour + 30-day gradient window, the §17.23 close-to-parent X on the focused-panel strip, the §17.24 plus-tile glyph rebuild — CSS pseudo-element cross at 38 / 11 cqmin in `--muted`, the §17.25 add-child modal two-pane layout — left-rail kind list + right-pane form + `availableKinds` seam, the §17.26 weight slider — `<input type="range" min=0 max=10 step=0.5>` + synced 5 rem numeric input both binding into the same `weight` state, and the §17.27 TextNode markdown rendering — zero-dep `markdownToHtml` (escape-first / safe-by-default) + tile-relative `.md-body` (cqmin clamp + JS shrink-to-fit between 8 / 64 px); DT-10 script half too — Tasks A/B remain manual ops); the working tree should be clean (or hold only docs/test-fixture WIP).
4. **Sanity check the build**: `npm test` (expect **540** tests across **45** files per §17.27), `npm run lint`, `npm run lint:rules`, `npm run build` (expect ~111 KB / ~33 KB gzip + a separate 0.75 KB `testBridge` chunk; **no Rollup warnings about cross-chunk imports**), and `npm run test:e2e` (expect **63** scenarios per §17.27) — all should be green before starting new work. **Important**: `npm run preview` (which `test:e2e` spawns) serves `dist/`, so a stale `dist/` after a `git pull` will silently make every e2e scenario fail. Always run `npm run build` after pulling. The first `npm run test:e2e` after a clone also needs `npx playwright install chromium`.
...
6. **Pick up the next strand**:
   ...
   - **Phase 9 polish (deferred from §17.20)**: a kinetic zoom-from-tile drill animation (read the tapped tile's bbox + transform the layout from there); an `encap--leave` class for the breadcrumb-tap inverse animation; keyboard-drill support if SPEC §1 ever lifts the "no keyboard" assumption.
